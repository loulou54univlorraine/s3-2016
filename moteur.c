...
void moteur_dessiner_Sprite(int i){
  ...
  double spriteDistance = ((Pos.x - sprite.x) * (Pos.x - sprite.x) + (Pos.y - sprite.y) * (Pos.y - sprite.y));
//translate sprite[i] position to relative to camera
  double spriteX = sprite.x - Pos.x;
  double spriteY = sprite.y - Pos.y;
  printf("%f  %f\n",spriteX ,spriteY);
   sprite.x=  sprite.x - 0.001*spriteX;
 sprite.y=  sprite.y - 0.001*spriteY;
